...
class GpsCheckSignalJobController extends Controller
{
    use GpsTrait;

    public function index(Request $request)
    {
        $this->authorize(Actions::View . '_' . Resources::GPSCheckSignalShortTerm);
        $user = Auth::user();
        ...
        return view('admin.gps-check-signal-jobs.index', [
            'list' => $list,
    ...
    public function edit(GpsCheckSignal $gps_check_signal_job)
    {
        $this->authorize(Actions::Manage . '_' . Resources::GPSCheckSignalShortTerm);
        $user = Auth::user();
        if (strcmp($gps_check_signal_job->job_type, Rental::class) === 0) {
            $rental = Rental::find($gps_check_signal_job->job_id);
            if ($rental) {
                $gps_check_signal_job->worksheet_no = ($rental->worksheet_no) ? $rental->worksheet_no : '';
                $gps_check_signal_job->service_type = ($rental && $rental->serviceType) ? __('gps.service_type_' . $rental->serviceType->service_type) : null;
                $gps_check_signal_job->customer = ($rental) ? $rental->customer_name : null;
                $gps_check_signal_job->pickup_date = ($rental) ? get_thai_date_format($rental->pickup_date, 'd/m/Y H:i') : null;
                $gps_check_signal_job->return_date = ($rental) ? get_thai_date_format($rental->return_date, 'd/m/Y H:i') : null;
            }
        }

        $car = Car::find($gps_check_signal_job->car_id);
        if ($car) {
            $gps_check_signal_job->license_plate = ($car) ? $car->license_plate : null;
            $gps_check_signal_job->engine_no = ($car) ? $car->engine_no : null;
            $gps_check_signal_job->chassis_no = ($car) ? $car->chassis_no : null;;
            $gps_check_signal_job->car_class = ($car && $car->carClass) ? $car->carClass->full_name : null;
            $gps_check_signal_job->car_color = ($car && $car->carColor) ? $car->carColor->name : null;
            $gps_check_signal_job->fleet = ($car) ? $car->fleet : null;
            $gps_check_signal_job->vid = ($car) ? $car->vid : null;
            $gps_check_signal_job->sim = ($car) ? $car->sim : null;
        }
        ...
        return view('admin.gps-check-signal-jobs.form',  [
            'page_title' => $page_title,
            'd' => $gps_check_signal_job,
    ...
    public function show(GpsCheckSignal $gps_check_signal_job)
    {
        $this->authorize(Actions::View . '_' . Resources::GPSCheckSignalShortTerm);
        $user = Auth::user();
        if (strcmp($gps_check_signal_job->job_type, Rental::class) === 0) {
    ...
    public function store(Request $request)
    {
        ...
        $redirect_route = route('admin.gps-check-signal-jobs.index');
        return $this->responseValidateSuccess($redirect_route);
    }

    public function sendCheckJob(Request $request)
    {
        if ($request->arr_send_check > 0) {
            foreach ($request->arr_send_check as $key => $item) {
                $send_check = GpsCheckSignal::find($item['id']);
                if ($send_check) {
                    $car = Car::find($send_check->car_id);
                    if ($car->vid) {
                        $vehicle_id = [
                            "VehicleList" => $car->vid,
                        ];
                                ...
                                foreach ($data as $d) {
                                    ...
                                    if ((!empty($vehicle_id))) {
                                        if (strcmp($vehicle_id, "222150159") == 0) {
                                            GpsCheckSignal::where('car_id', $car->id)->update([
                                                'status' => GPSStatusEnum::NORMAL_SIGNAL,
                                            ]);
                                        } else {
                                            if (!$io_status) {
                                                GpsCheckSignal::where('car_id', $car->id)->update([
                                                    'status' => GPSStatusEnum::NO_SIGNAL,
                                                ]);
                                            } else {
                                                GpsCheckSignal::where('car_id', $car->id)->update([
                                                    'status' => GPSStatusEnum::NORMAL_SIGNAL,
                        ...
                        $send_check->status = GPSStatusEnum::CHECK_SIGNAL;
                        $send_check->save();
                    }
                }
            }
    ...
    public function sendBranchJob(Request $request)
    {
        if ($request->arr_send_branch > 0) {
            foreach ($request->arr_send_branch as $key => $item) {
                $send_check = GpsCheckSignal::find($item['id']);
                if ($send_check) {
                    $send_check->check_main_branch = BOOL_TRUE;
                    $send_check->save();
                }
            }
        }

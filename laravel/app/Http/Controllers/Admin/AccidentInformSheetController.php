...
class AccidentInformSheetController extends Controller
{
    public function index(Request $request)
    {
        $this->authorize(Actions::View . '_' . Resources::AccidentInformSheet);
        ...
        return view('admin.accident-informs.index-sheet', [
            'list' => $list,
    ...
    {
        $this->authorize(Actions::Manage . '_' . Resources::AccidentInformSheet);
        $d = new Accident();
    ...
    public function store(Request $request)
    {
        ...
        $redirect_route = route('admin.accident-inform-sheets.index');
        return $this->responseValidateSuccess($redirect_route);
    }


    public function show(Accident $accident_inform_sheet)
    {
        $this->authorize(Actions::View . '_' . Resources::AccidentInformSheet);
        $car = Car::find($accident_inform_sheet->car_id);
        $car_license = null;
        if ($car) {
            if ($car->license_plate) {
        ...
        $car_data = RepairTrait::getDataCar($accident_inform_sheet->car_id);
        $rental = $car_data->rental;
        $condotion_lt_rental =  RepairTrait::getConditionQuotation($accident_inform_sheet);
        $need_list = AccidentTrait::getNeedList();
        $replace_list = AccidentTrait::getReplacementList();
        $province_list = AccidentTrait::getProvinceList();
        $case_list = AccidentTrait::getCaseList();
        ...
        $province = Province::find($accident_inform_sheet->province_id);
        $province_name = $province->name_th;
        $amphure =  Amphure::find($accident_inform_sheet->district_id);
        $amphure_name = $amphure ? $amphure->name_th : null;
        $district =  District::find($accident_inform_sheet->subdistrict_id);
        $district_name =  $district ? $district->name_th : null;

        $cradle = Cradle::find($accident_inform_sheet->cradle);

        $replacement_car_files = $accident_inform_sheet->getMedia('replacement_car_files');
        $replacement_car_files = get_medias_detail($replacement_car_files);

        $slide_list = $this->getSlideList($accident_inform_sheet);
        $cost_list = $this->getCostList($accident_inform_sheet);

        $replacement_list = $this->getReplacementList($accident_inform_sheet);
        $cost_list = $this->getCostList($accident_inform_sheet);
        $accident_slide_list = AccidentTrait::getAccidentSlideList();

        $receive_status_list = $this->getStatusReceiveList();

        $slide_worksheet_list = Slide::where('job_type', Accident::class)->where('job_id', $accident_inform_sheet->id)->select('id', 'worksheet_no as name')->get();
    ...
    public function edit(Accident $accident_inform_sheet)
    {
        $this->authorize(Actions::Manage . '_' . Resources::AccidentInformSheet);
        $car = Car::find($accident_inform_sheet->car_id);
        $car_license = null;
    ...
    public function editClaim(Accident $accident_inform_sheet)
    {
        $this->authorize(Actions::Manage . '_' . Resources::AccidentInformSheet);
        $repair_list = [];
        ...
        $claim_type_list = AccidentTrait::getCliamTypeList();
        $getClaimList = $this->getClaimList($accident_inform_sheet);
    ...
    public function showClaim(Accident $accident_inform_sheet)
    {
        $this->authorize(Actions::View . '_' . Resources::AccidentInformSheet);
        $repair_list = [];
        $spare_part_list = $this->getStatusList();
    ...
    public function storeEditClaim(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'reason_except_deductible' => [Rule::when($request->is_except_deductible == RightsEnum::NOT_USE_RIGHTS, ['required'])],
        ...
        if ($accident_informs) {
            ...
            if (strcmp($accident_informs->status, AccidentStatusEnum::WAITING_CLAIM) === 0) {
                $accident_informs->status = AccidentStatusEnum::WAITING_OPEN_ORDER;
            }
            $accident_informs->save();
    ...
    public function createCarReplacement($accident, $request)
    {
        ...
        $replacement_car->replacement_date = $request->replacement_expect_date;
    ...
    public function createCarReplacementEdit(Request $request)
    {
        ...
        $redirect_route = route('admin.accident-inform-sheets.edit', ['accident_inform_sheet' => $request['accident_id']]);
        return $this->responseValidateSuccess($redirect_route);
        // return route('admin.accident-inform-sheets.edit', ['accident_inform_sheet' => $request['accident_id']]);
    }

...
class GpsCheckSignalJobReplacementController extends Controller
{
    use GpsTrait;
    public function index(Request $request)
    {
        $this->authorize(Actions::View . '_' . Resources::GPSCheckSignalReplacement);
        $user = Auth::user();
        ...
        $list = GpsCheckSignal::leftJoin('cars', 'cars.id', '=', 'gps_check_signals.car_id')
            ...
            ->where('gps_check_signals.job_type', ReplacementCar::class)
        ...
        $list->map(function ($item) {
            $item->rental_date = '';
            if (strcmp($item->job_type, ReplacementCar::class) == 0) {
                $replacement_car = ReplacementCar::find($item->job_id);
                $item->replacement_date = ($replacement_car) ? get_thai_date_format($replacement_car->replacement_date, 'd/m/Y') : null;
            }
            return $item;
        });
        ...
        return view('admin.gps-check-signal-job-replaces.index', [
            'list' => $list,
    ...
    public function edit(GpsCheckSignal $gps_check_signal_job_replace)
    {
        $this->authorize(Actions::Manage . '_' . Resources::GPSCheckSignalReplacement);
        $user = Auth::user();
        if (strcmp($gps_check_signal_job_replace->job_type, ReplacementCar::class) === 0) {
            $replacement_car = ReplacementCar::find($gps_check_signal_job_replace->job_id);
            if ($replacement_car) {
                $gps_check_signal_job_replace->worksheet_no = ($replacement_car) ? $replacement_car->worksheet_no : null;
                $gps_check_signal_job_replace->replacement_type = ($replacement_car) ? __('replacement_cars.type_' . $replacement_car->replacement_type) : null;
                $gps_check_signal_job_replace->replacement_customer = ($replacement_car) ? $replacement_car->customer_name : null;
                $gps_check_signal_job_replace->replacement_date = ($replacement_car) ? get_thai_date_format($replacement_car->replacement_date, 'd/m/Y') : null;
                $gps_check_signal_job_replace->replacement_place = ($replacement_car) ? $replacement_car->replacement_place : null;
            }
        }
        $car = Car::find($gps_check_signal_job_replace->car_id);
        if ($car) {
            $gps_check_signal_job_replace->license_plate = ($car) ? $car->license_plate : null;
            $gps_check_signal_job_replace->engine_no = ($car) ? $car->engine_no : null;
            $gps_check_signal_job_replace->chassis_no = ($car) ? $car->chassis_no : null;;
            $gps_check_signal_job_replace->car_class = ($car && $car->carClass) ? $car->carClass->full_name : null;
            $gps_check_signal_job_replace->car_color = ($car && $car->carColor) ? $car->carColor->name : null;
            $gps_check_signal_job_replace->fleet = ($car) ? $car->fleet : null;
            $gps_check_signal_job_replace->vid = ($car) ? $car->vid : null;
            $gps_check_signal_job_replace->sim = ($car) ? $car->sim : null;
        }
        ...
        return view('admin.gps-check-signal-job-replaces.form',  [
            'page_title' => $page_title,
            'd' => $gps_check_signal_job_replace,
    ...
    public function show(GpsCheckSignal $gps_check_signal_job_replace)
    {
        $this->authorize(Actions::View . '_' . Resources::GPSCheckSignalReplacement);
        if (strcmp($gps_check_signal_job_replace->job_type, ReplacementCar::class) === 0) {
            $replacement_car = ReplacementCar::find($gps_check_signal_job_replace->job_id);
    ...
    public function store(Request $request)
    {
        ...
        $redirect_route = route('admin.gps-check-signal-job-replaces.index');

...
class Select2GarageController extends Controller
{
    ...
    function getCradleByProvince(Request $request)
    {
        // dd($request->parent_id);
        $list = Cradle::select('cradles.id', 'cradles.name')
            // ->leftJoin('amphures', 'amphures.id', '=', 'districts.amphure_id')
            ->where(function ($query) use ($request) {
                if (!empty($request->s)) {
                    $query->where('cradles.name', 'like', '%' . $request->s . '%');
                }
                if (!empty($request->parent_id)) {
                    $query->where('cradles.province', $request->parent_id);
                }
            })
            ->get()->map(function ($item) {
                return [

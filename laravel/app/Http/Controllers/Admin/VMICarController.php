...
class VMICarController extends Controller
{
    public function index(Request $request)
    {
        $this->authorize(Actions::View . '_' . Resources::VMI);
        ...
        return view('admin.vmi-cars.index', [
            'list' => $list,
            's' => $request->s,
            'vmi_id' => $vmi_id,
    ...
    public function edit(VMI $vmi_car)
    {
        $this->authorize(Actions::Manage . '_' . Resources::VMI);
        ...
        return view('admin.vmi-cars.form', [
            'mode' => MODE_UPDATE,
    ...
    public function show(VMI $vmi_car)
    {
        $this->authorize(Actions::View . '_' . Resources::VMI);
        $car = Car::find($vmi_car->car_id);
        if ($car) {
        ...
        $premium_summary = InsuranceTrait::summaryPremium($vmi_car->premium, $vmi_car->discount, $vmi_car->stamp_duty, $vmi_car->tax);
        $vmi_car->sum_insured_total = number_format($vmi_car->sum_insured_car + $vmi_car->sum_insured_accessory, 2, '.', ',');
        $page_title = __('vmi_cars.page_title');
        return view('admin.vmi-cars.form', [
            'mode' => MODE_VIEW,
            'page_title' => $page_title,
            'd' => $vmi_car,
    ...
    public function store(Request $request)
    {
        ...
        if (strcmp($vmi_status, InsuranceStatusEnum::PENDING) === 0) {
            if ($vmi->term_start_date && $vmi->term_end_date) {
        ...
        $redirect_route = route('admin.vmi-cars.index');
    ...
    public function importVMIs(Request $request)
    {
        ...
        foreach ($data[0] as $row) {
            ...
            $vmi->repair_type = $type_of_repair;
            $vmi->remark = $remark;

...
class ConditionQuotationLongTermRentalController extends Controller
{
    public function index(Request $request)
    {
        $this->authorize(Actions::View . '_' . Resources::LongTermConditionQuotation);
        $condition_group = ConditionQuotationTrait::getConditionGroup(ConditionGroupEnum::LONG_TERM_RENTAL);
        $list = [];
        if ($condition_group) {
        ...
        $create_route = 'admin.condition-quotation-long-terms.create';
        $edit_route = 'admin.condition-quotation-long-terms.edit';
        $show_route = 'admin.condition-quotation-long-terms.show';
        $delete_route = 'admin.condition-quotation-long-terms.destroy';
        $param = 'condition_quotation_long_term';
        $view_permission = Actions::View . '_' . Resources::LongTermConditionQuotation;
        $manage_permission = Actions::Manage . '_' . Resources::LongTermConditionQuotation;
        $page_title = __('condition_quotations.long_term_page_title');
    ...
    {
        $this->authorize(Actions::Manage . '_' . Resources::LongTermConditionQuotation);
        $condition_group = ConditionQuotationTrait::getConditionGroup(ConditionGroupEnum::LONG_TERM_RENTAL);
        $d = new ConditionQuotation();
        $d->status = STATUS_ACTIVE;
        $d->condition_type = LongTermRentalApprovalTypeEnum::AFFILIATED;
        $d->condition_group_id = ($condition_group) ? $condition_group->id : null;
        $sub_condition_checklist = new ConditionQuotationChecklist();
        $list_type = ConditionQuotationTrait::getLongTermRentalApprovalType();
        $page_title =  __('lang.create') . __('condition_quotations.long_term_page_title');
        $redirect_route = route('admin.condition-quotation-long-terms.index');
        $store_route = route('admin.condition-quotation-long-terms.store');
        $manage_permission = Actions::Manage . '_' . Resources::LongTermConditionQuotation;
    ...
    public function edit(ConditionQuotation $condition_quotation_long_term)
    {
        $this->authorize(Actions::Manage . '_' . Resources::LongTermConditionQuotation);
        $sub_condition_checklist = ConditionQuotationChecklist::where('condition_quotations_id', $condition_quotation_long_term->id)->orderBy('seq', 'asc')->get();
        $sub_condition_checklist->map(function ($item) {
            $item->status  = $item->status == STATUS_INACTIVE ? false : true;
            return $item;
        });
        $list_type = ConditionQuotationTrait::getLongTermRentalApprovalType();
        $page_title =  __('lang.edit') . __('condition_quotations.long_term_page_title');
        $redirect_route = route('admin.condition-quotation-long-terms.index');
        $store_route = route('admin.condition-quotation-long-terms.store');
        $manage_permission = Actions::Manage . '_' . Resources::LongTermConditionQuotation;
        return view('admin.condition-quotations.form', [
            'd' => $condition_quotation_long_term,
            'page_title' => $page_title,
            'sub_condition_checklist' => $sub_condition_checklist,
            'list_type' => $list_type,
            'redirect_route' => $redirect_route,
    ...
    public function show(ConditionQuotation $condition_quotation_long_term)
    {
        $this->authorize(Actions::View . '_' . Resources::LongTermConditionQuotation);
        $sub_condition_checklist = ConditionQuotationChecklist::where('condition_quotations_id', $condition_quotation_long_term->id)->orderBy('seq', 'asc')->get();
        $sub_condition_checklist->map(function ($item) {

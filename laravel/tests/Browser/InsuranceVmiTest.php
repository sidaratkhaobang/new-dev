...
class InsuranceVmiTest extends DuskTestCase
{
    /**
     * A Dusk test example.
     *
    ...
    public function test_TC_PROVIDE_INSURANCE_01()
    {
        $this->browse(function (Browser $browser) {
            $browser
                ->loginAs(User::where('username', $this->username)->first())
                ->visit('/admin/vmi-cars?status=PENDING')
                ->pause(1000)
                ->assertSee('งานประกัน')
                ...
                ->select('#insurance_type', $this->insurance_type)
                ->select2('#insurance_package_id', $this->insurance_package_id)
...
//            $browser->script('window.scrollTo(0, 1500);');
            $browser->screenshot('vmi');
        });
    }

    public function test_TC_PROVIDE_INSURANCE_02()
    {
        $this->browse(function (Browser $browser) {
            $browser
                ->loginAs(User::where('username', $this->username)->first())
            ...
            $browser->jsClick('#saveDetail')
                ...
                ->assertSeeIn('.swal2-container', 'บันทึกข้อมูลเรียบร้อย')
                ->press('ตกลง');
            $browser->screenshot('vmi_checkbox');
        });
    }

    public function test_TC_RECORD_INFORMATION_AFTER_INSURANCE()
    {
        $this->browse(function (Browser $browser) {
            $browser
                ->loginAs(User::where('username', $this->username)->first())
                ->visit('/admin/vmi-cars?status=IN_PROCESS')
                ->assertSee('งานประกัน')
                ->click('#dropdown-dropleft-dark')
                ->clickLink("แก้ไข")

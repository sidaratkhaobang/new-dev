...
@section('page_title', __('cmi_cars.cancel_title'))
@push('styles')
    <style>
        .seperator {
            font-size: xx-large;
...
@section('block_options_2')
    @can(Actions::View . '_' . Resources::CancelCMI)
        <div class="block-options-item">
            <button class="btn btn-success" onclick="exportCancelInsuranceList('{{ __('cmi_cars.cancel_cmi_file_title') }}')">
                <i class="icon-document-download me-1"></i> {{ __('cmi_cars.download') }}</button>
        </div>
        <div class="block-options-item seperator">|</div>
    @endcan
    @can(Actions::Manage . '_' . Resources::CancelCMI)
        <div class="block-options-item">
            ...
            <div class="justify-content-between mb-4">
                <form action="" method="GET" id="form-search">
                    <div class="form-group row push mb-4">
                        <div class="col-sm-3">
                            <x-forms.select-option :value="$cancel_id" id="cancel_id" :list="null"
                        ...
                        @if (sizeof($list) > 0)
                            ...
                            <tr>
                                ...
                                <td class="sticky-col text-center">
                                    @include('admin.cancel-cmi-cars.sections.dropdown-actions')
                                </td>
                            </tr>
                            @endforeach
                        @else 
...
@include('admin.components.select2-ajax', [
    'id' => 'cancel_id',
    'url' => route('admin.util.select2-cmi.cancel-cmi-worksheets'),
])

@include('admin.components.select2-ajax', [
    'id' => 'license_plate',
    'url' => route('admin.util.select2-cmi.cancel-cmi-license-plates'),
])

@include('admin.components.select2-ajax', [
    'id' => 'insurer_id',
    'url' => route('admin.util.select2-cmi.cancel-cmi-insurers'),
])
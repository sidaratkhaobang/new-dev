...
@section('page_title', __('accident_follow_up_repairs.page_title'))
        ...
        {{-- <div class="block-options-item">
            @can(Actions::Manage . '_' . Resources::AccidentFollowUpRepair)
                <x-btns.add-new btn-text="{{ __('garages.add_new') }}"
                    route-create="{{ route('admin.accident-orders.create') }}" />
            @endcan
        </div> --}}
    </div>
@endsection
                    ...
                    <div class="form-group row push mb-4">
                        <div class="col-sm-3">
                            <x-forms.select-option :value="$accident_worksheet_no" id="accident_worksheet_no" :list="null"
                                :label="__('accident_follow_up_repairs.accident_worksheet_no_ref')" :optionals="[
                        ...
                        <div class="col-sm-3">
                            <x-forms.select-option :value="$license_plate" id="license_plate" :list="null" :label="__('accident_orders.license_plate')"
                                :optionals="[
                                    'select_class' => 'js-select2-custom',
                                    'ajax' => true,
                ...
                <table class="table table-striped table-vcenter">
                    <thead class="bg-body-dark">
                        <tr>
                            ...
                            <th style="width: 25%;">@sortablelink('accident_worksheet', __('accident_follow_up_repairs.accident_worksheet_no_ref'))</th>
                            <th style="width: 25%;">@sortablelink('license_plate', __('accident_informs.license_plate'))</th>
                            <th style="width: 25%;">@sortablelink('cradle_name', __('accident_follow_up_repairs.garage'))</th>
                            <th style="width: 25%;">@sortablelink('insurance_name', __('accident_orders.insurance_name'))</th>   
                            ...
                            <th style="width: 25%;">{{ __('accident_orders.over_complete_date') }}</th>
                            <th style="width: 25%;">{{__('accident_orders.follow_repair')}}</th>
                            <th class="text-center" style="width: 25%;">@sortablelink('status', __('accident_informs.status'))</th>
                        ...
                        @if (!$list->isEmpty())
                            @foreach ($list as $index => $d)
                                <tr>
                                    ...
                                    <td>{{ $d->scheduled_completion_date ? $d->scheduled_completion_date : '' }}
                                    </td>
                                    <td>{{ $d->over_complete_date }}</td>
                                    </td>
                                    <td>{{ $d->follow_up_status ?  $d->follow_up_status : null }}</td>
                                    ...
                                    <td class="sticky-col text-center">
                                        {{-- @if (strcmp($d->status, AccidentRepairStatusEnum::SUCCESS_REPAIR) !== 0) --}}
                                            @include('admin.components.dropdown-action', [
                                                'edit_route' => route('admin.accident-follow-up-repairs.edit', [
                                                    'accident_follow_up_repair' => $d,
                                                ]),
                                                'view_route' => route('admin.accident-follow-up-repairs.show', [
                                                    'accident_follow_up_repair' => $d,
                                                ]),
                                                'view_permission' =>
                                                    Actions::View . '_' . Resources::AccidentFollowUpRepair,
                                                'manage_permission' =>
                                                    Actions::Manage . '_' . Resources::AccidentFollowUpRepair,
                                            ])
                                        {{-- @else
                                            @include('admin.components.dropdown-action', [
                                                'view_route' => route('admin.accident-follow-up-repairs.show', [

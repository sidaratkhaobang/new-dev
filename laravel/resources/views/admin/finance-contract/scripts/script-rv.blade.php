    ...
    <script>
        let VrDetailVue = new Vue({
            ...
            data: {
                ...
                car_accessory_total: @if(isset($total_accessory_price)) {{$total_accessory_price ?? 0}} @else 0 @endif,
                rv_car_percent: @if(isset($d?->rv_car_percent)) {{$d?->rv_car_percent ?? 0}} @else 0 @endif,
                rv_car_accessory_percent: @if(isset($d?->rv_accessory_percent)) {{$d?->rv_accessory_percent ?? 0}} @else 0 @endif,
            ...
            watch: {
                ...
                rv_car_accessory_percent: function (newVal, oldVal) {
                    if (newVal) {
                        let calculate = (parseFloat(this.car_accessory_total) * parseFloat(newVal)) / 100
                        this.rv_car_accessory_summary_total = calculate.toLocaleString()
                    } else {
                        this.rv_car_accessory_summary_total = 0
                    }

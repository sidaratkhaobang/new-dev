...
@section('content')
    ...
    <form id="save-form">
        ...
        @include('admin.tax-renewals.section-taxings.tax-renew-detail')
        @include('admin.tax-renewals.section-taxings.proceed-detail')
        @include('admin.tax-renewals.section-waiting-send-tax-register-books.send-tax-detail')
        @include('admin.tax-renewals.section-waiting-send-tax-register-books.return-register-book-detail')
...
@include('admin.components.form-save', [
    'store_uri' => route('admin.tax-renewals.store-waiting-send-tax-register-book'),
])
...
@push('scripts')
    <script>
        ...
        $("#send_tax_renew_date").prop('disabled', true);
        $("#provider").prop('disabled', true);
        $("#amount_tax_renew_date").prop('disabled', true);

        $("#receive_tax_label_date").prop('disabled', true);
        $("#car_tax_exp_date").prop('disabled', true);
        ...
        $(".js-flatpickr-icon").css("background-color", "#f1f1f1");
        ...
        $(document).ready(function() {
            ...
            //
            var send_tax_renew_date = new Date($('#send_tax_renew_date').val());
            var receive_tax_label_date = new Date($('#receive_tax_label_date').val());

            // amount_day_wait_cmi
            $('#send_tax_renew_date , #receive_tax_label_date').on('change', function() {
                var receive_tax_label_date = new Date($('#receive_tax_label_date').val());
                var send_tax_renew_date = new Date($('#send_tax_renew_date').val());
                var day_diff = DateDifference(send_tax_renew_date, receive_tax_label_date);
                $('#amount_tax_renew_date').val(day_diff);
            });

            var amount_tax_renew_date = DateDifference(send_tax_renew_date,
                receive_tax_label_date);
            $('#amount_tax_renew_date').val(amount_tax_renew_date);
        ...
        $(".btn-save-form-tax-renewal").on("click", function() {
            let storeUri = "{{ route('admin.tax-renewals.store-waiting-send-tax-register-book') }}";
        ...
        jQuery(function() {
            var receive_tax_label_date = '{{ $d->receive_tax_label_date }}';
            flatpickr("#send_tax_date", {
                minDate: receive_tax_label_date,
            });
            flatpickr("#return_registration_book_date", {
                minDate: receive_tax_label_date,
            });
        });
    </script>

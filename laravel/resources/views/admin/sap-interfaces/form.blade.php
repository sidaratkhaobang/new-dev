    ...
    <div class="block {{ __('block.styles') }}">
        <div class="block-content">
            <form id="save-form">
                        ...
                        <tbody>
                            @foreach ($sap_interface_lines as $index => $d)
                                <tr>
                                    ...
                                    <td>{{ $d->posting_key }}</td>
                                    <td>{{ $d->account_no }}</td>
                                    <td>{{ number_format($d->amount_in_document,2) }}</td>
                                    <td>{{ number_format($d->amount_in_document, 2) }}</td>
                                    <td>{{ $d->cost_center }}</td>
                                    <td></td>
                                    <td>{{ !empty($d->base_amount)?number_format($d->base_amount,2):null  }}</td>
                                    <td>{{ $d->tax_code }}</td>
                                    <td>{{ $d->assignment }}</td>
                                    <td>{{ $d->text }}</td>
                                </tr>
                ...
                <x-forms.submit-group :optionals="['url' => 'admin.sap-interfaces.index', 'view' => empty($view) ? null : $view]" />
            </form>
        </div>
    </div>
@endsection

<template v-for="(line,j) in rental_line">
    ...
    <div class="col-sm-2 mt-4 align-self-center">
        <input type="checkbox" class="col-form-label form-check-input me-2" 
            :id="'checkbox-inline-' + line.id " :name="'no_car_dealer['+ line.id +']'"
            :data-value="line.id"
            v-model="line.no_cars_checked">
        {{ __('long_term_rentals.no_car_dealer') }}
    </div>
    <div class="col-sm-4 mt-4">
        <div class="row">
            <label class="col-sm-2 col-form-label" for="example-hf-email">{{ __('long_term_rentals.no_car_reason') }}</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" 
                :id="'no_car_reason-' + line.id " :name="'no_car_reason['+ line.id +']'"
                :data-value="line.id"
                v-model="line.no_car_reason">
            </div>
        </div>
      </div>
    ...
    <div class="table-wrap mb-2">
        ...
        @if (empty($view))
            <div class="col-md-12 text-end">
                <button v-if="!line.no_cars_checked" type="button" class="btn btn-primary" v-on:click="addSelect(j)">{{ __('lang.add') }}</button>
            </div>
        @endif
    </div>
</template>

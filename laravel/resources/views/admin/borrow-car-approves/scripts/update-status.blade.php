@push('scripts')
    <script>
        function updateBorrowCarStatus(data) {
            var updateUri = "{{ route('admin.borrow-car-approves.update-status') }}";
        ...
        $(".btn-not-approve-status").on("click", function () {
            var data = {
                bc_status: $(this).attr('data-status'),
                borrow_car: $(this).attr('data-id'),
                redirect: "{{ route('admin.borrow-car-approves.index') }}",
                approve_line_id: document.getElementById("approve_line").value,
            };
            mySwal.fire({
                title: 'ยืนยันไม่อนุมัติใบงานยืมรถ',
                html: 'กรุณาให้เหตุผลการไม่อนุมัติใบงานยืมรถในครั้งนี้ <span class="text-danger">*</span>',
                input: 'text',
                icon: 'warning',
                customClass: {
                    confirmButton: 'btn btn-danger m-1',
        ...
        $(".btn-approve-status").on("click", function () {
            ...
            mySwal.fire({
                title: 'ยืนยันอนุมัติใบงานยืมรถ',
                html: 'เมื่อยืนยันใบงานยืมรถ ระบบจะส่งข้อมูลคำขอนี้เพื่อดำเนินการในส่วนต่อไป',
                icon: 'warning',
                customClass: {

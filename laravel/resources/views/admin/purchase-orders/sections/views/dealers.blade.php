                ...
                @if (sizeof($purchase_order_dealer_list) > 0)
                    <tr>
                        @foreach ($purchase_order_dealer_list as $i => $d)
                            ...
                            <th class="custom-border-right text-end">{{ __('purchase_orders.discount_per_car') }}</th>
                        ...
                        <tr>
                            ...
                            @foreach ($purchase_order_dealer_list as $purchase_order_dealer_index => $purchase_order_dealer)
                                <td class="text-end custom-border-right">
                                    {{ isset($purchase_order_dealer->dealer_price_list[$car_index]) ? number_format($purchase_order_dealer->dealer_price_list[$car_index]->car_price, 2) : 0 }}
                                </td>
                                <td class="text-end custom-border-right">
                                    {{ isset($purchase_order_dealer->dealer_price_list[$car_index]) ? number_format($purchase_order_dealer->dealer_price_list[$car_index]->discount, 2) : 0 }}
                                </td>
                                <td class="text-end custom-border-right">
                                    {{ isset($purchase_order_dealer->dealer_price_list[$car_index]) ? number_format($purchase_order_dealer->dealer_price_list[$car_index]->car_price * $car->amount, 2) : 0 }}
                                </td>
                            @endforeach
                        </tr>
                    @endforeach

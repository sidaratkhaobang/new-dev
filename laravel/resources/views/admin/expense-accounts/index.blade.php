...
@section('page_title', __('sap_interfaces.expense_account'))
            ...
            <div class="justify-content-between">
                <form action="" method="GET" id="form-search">
                    <div class="form-group row push mb-4">
                        ...
                        <div class="col-sm-3">
                            <x-forms.select-option id="transfer_sub_type" :value="$transfer_sub_type" :list="$transfer_sub_type_list"
                                :label="__('sap_interfaces.transfer_sub_type')" />
                        </div>
                        <div class="col-sm-3">
                            <x-forms.select-option id="doc_type_id" :value="$doc_type_id" :list="$document_type_list" :label="__('sap_interfaces.doc_type')" />
                        </div>
                        <div class="col-sm-3">
                        ...
                        @foreach ($list as $index => $d)
                            <tr>
                                ...
                                <td class="text-center">
                                    {!! $d->status
                                        ? badge_render(__('sap_interfaces.class_' . $d->status), __('sap_interfaces.status_' . $d->status))
                                        : null !!}
                                </td>
                                <td class="sticky-col text-center">
                                    @include('admin.components.dropdown-action', [
                                        'view_route' => route('admin.expense-accounts.show', [
                                            'expense_account' => $d,
                                        ]),
                                        'view_permission' => Actions::View . '_' . Resources::SAPInterfaceAP,
                                    ])
                                </td>
                            </tr>
                        @endforeach

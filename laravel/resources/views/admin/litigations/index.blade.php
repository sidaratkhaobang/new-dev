...
@section('page_title', __('litigations.page_title'))

@section('block_options_list')
    <div class="block-options-item">
        @can(Actions::Manage . '_' . Resources::Litigation)
            <x-btns.add-new btn-text="{{ __('litigations.add_new') }}" route-create="{{ route('admin.litigations.create') }}" />
        @endcan
    </div>
@endsection
@section('content')
                            ...
                            @foreach ($list as $index => $d)
                                <tr>
                                    ...
                                    <td class="sticky-col text-center">
                                        @if (in_array($d->status, [LitigationStatusEnum::PENDING_REVIEW, LitigationStatusEnum::COMPLETE, LitigationStatusEnum::CONFIRM]))
                                            <x-tables.dropdown 
                                                :view-route="route('admin.litigations.show', ['litigation' => $d])" 
                                                :view-permission="Actions::View . '_' . Resources::Litigation" >
                                            </x-tables.dropdown>
                                        @else
                                            <x-tables.dropdown 
                                                :view-route="route('admin.litigations.show', ['litigation' => $d])" 
                                                :edit-route="route('admin.litigations.edit', ['litigation' => $d])" 
                                                :view-permission="Actions::View . '_' . Resources::Litigation"
                                                :manage-permission="Actions::Manage . '_' . Resources::Litigation" >
                                            </x-tables.dropdown>
                                        @endif
                                    </td>
                                </tr>

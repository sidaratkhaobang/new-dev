    ...
    @if (!isset($view_only))
        @if (!isset($accessory_controller))
            <div class="col-8 text-end">
                <button type="button" id="add-dealer" class="btn btn-primary" onclick="addDealer()">เพิ่ม Dealer</button>
            </div>
        @endif
    @endif
</div>
...
<div>
    <div class="table-wrap">
        <table class="table table-striped">
            ...
            @if (count($car_list) > 0)
                <tbody>
                    @foreach ($car_list as $index => $tor_line)
                        <tr>
...
<div class="col-auto mt-2">
    ...
    <label for="check_delivery_ready">
        <input type="radio" id="check_delivery_ready" class="form-check-input radio" name="check_delivery"
            value="{{ STATUS_ACTIVE }}" @if ($d->check_delivery == STATUS_ACTIVE && !is_null($d->check_delivery)) checked @endif>
        <span>&nbsp;มีรถพร้อมส่งมอบ</span>
    </label> &emsp;
    <label for="check_delivery_no_ready">
        <input type="radio" id="check_delivery_no_ready" class="form-check-input radio" name="check_delivery"
            value="{{ STATUS_DEFAULT }}" @if ($d->check_delivery == STATUS_DEFAULT && !is_null($d->check_delivery)) checked @endif>
        <span>&nbsp;ไม่มีรถพร้อมส่งมอบ</span>
    </label> &emsp;
    <label for="check_delivery_change">
        <input type="radio" id="check_delivery_change" class="form-check-input radio" name="check_delivery"
            value="{{ STATUS_INACTIVE }}" @if ($d->check_delivery == STATUS_INACTIVE && !is_null($d->check_delivery)) checked @endif>
        <span>&nbsp;เปลี่ยนรถ</span>
    </label>
</div>

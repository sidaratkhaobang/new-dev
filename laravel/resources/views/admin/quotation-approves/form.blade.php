                ...
                <div class="row push">
                    <div class="col-sm-12 text-end">
                        <a class="btn btn-secondary"
                            href="{{ route('admin.quotation-approves.index') }}">{{ __('lang.back') }}</a>

                        @if (empty($view))
                            @can(Actions::Manage . '_' . Resources::QuotationApprove)
                        ...
                        {{-- รออนุมัติ --}}
                        @if ($approve_line_owner)
                            @if ($d->status == QuotationStatusEnum::PENDING_REVIEW)
                                @can(Actions::Manage . '_' . Resources::QuotationApprove)
                                    <button type="button" class="btn btn-danger btn-disapprove-status"
                                        data-id="{{ $d->id }}"
                                        data-status="{{ QuotationStatusEnum::REJECT }}">{{ __('lang.disapprove') }}
                                    </button>
                                    <button type="button" class="btn btn-primary btn-approve-status"
                                        data-id="{{ $d->id }}"
                                        data-status="{{ QuotationStatusEnum::CONFIRM }}">{{ __('purchase_requisitions.approved') }}</button>
                                @endcan
                            @endif
                        @endif
                    </div>
...
@include('admin.quotation-approves.scripts.update-status')
{{-- @include('admin.components.form-save', [
    'store_uri' => route('admin.quotation-approves.updates-status'),
]) --}}

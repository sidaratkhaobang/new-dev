@push('scripts')
    ...
    <script>
        $(document).ready(function(){
            console.log(GanttChartVue.car_list_temp)
        })
        ...
        let GanttChartVue = new Vue({
            ...
            data: {
                ...
                daysName: [],
                car_brand_id: null,
                service_type_id: @if(!empty($idServiceType)) @json($idServiceType) @else null @endif,
                rental_id: @if(!empty($rental_id)) @json($rental_id) @else null @endif ,
                // offsetDays: moment().startOf("month").day() - 1,
                currentMonthName: moment().format("MMMM Y"),
                select_multiple: @if(!empty($select_multiple)) @json($select_multiple) @else null @endif ,
                pickup_date: @json($d->pickup_date),
                return_date: @json($d->return_date),
                car_rental : @if(!empty($car_rental)) @json($car_rental) @endif,
                // search: null,

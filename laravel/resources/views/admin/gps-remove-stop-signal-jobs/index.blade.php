...
@section('content')
    ...
    <div class="block {{ __('block.styles') }}">
        ...
        <div class="block-content">
            ...
            {{--                                    @can(Actions::Manage . '_' . Resources::GPSRemoveStopSignalJob) --}}
            {{--                                        <button class="btn btn-primary" onclick="openModalSendRemove()"><i class="fa fa-signal"></i> --}}
            {{--                                            {{ __('gps.send_remove_gps') }}</button> --}}
            {{--                                    @endcan --}}
            {{--                                    @can(Actions::Manage . '_' . Resources::GPSRemoveStopSignalJob) --}}
            {{--                                        <button class="btn btn-purple" onclick="openModalSendStop()"><i class="fa fa-broadcast-tower"></i> --}}
            {{--                                            {{ __('gps.send_stop_gps') }}</button> --}}
            {{--                                    @endcan --}}
            {{--                                </div> --}}
            {{--                </div> --}}
            {{--            </div> --}}
            <div class="justify-content-between">
                <form action="" method="GET" id="form-search">
                    ...
                    <div class="form-group row push mb-4">
                        <div class="col-sm-3">
                            <x-forms.date-input id="remove_date" :value="$remove_date" :label="__('gps.remove_date')" :optionals="['placeholder' => __('lang.select_date')]" />
                        </div>
                        <div class="col-sm-3">
                        ...
                        <div class="col-sm-3">
                            <x-forms.date-input id="stop_date" :value="$stop_date" :label="__('gps.stop_date')" :optionals="['placeholder' => __('lang.select_date')]" />
                        </div>
                        <div class="col-sm-3">
    ...
    @section('block_options_list')
        <div class="block-options-item">
            @can(Actions::Manage . '_' . Resources::GPSRemoveStopSignalJob)
                <button class="btn btn-primary me-1" onclick="openModalSendRemove()"><i class="fa fa-signal"></i>
                    {{ __('gps.send_remove_gps') }}</button>
            @endcan
            @can(Actions::Manage . '_' . Resources::GPSRemoveStopSignalJob)
                <button class="btn btn-purple" onclick="openModalSendStop()"><i class="fa fa-broadcast-tower"></i>
    ...
    <div class="block-content">
                                ...
                                <td class="sticky-col text-center">
                                    @if (strcmp($d->remove_status, GPSStopStatusEnum::WAIT_REMOVE_GPS) == 0 ||
                                        ...
                                        @include('admin.components.dropdown-action', [
                                            'view_route' => route('admin.gps-remove-stop-signal-jobs.show', [
                                                'gps_remove_stop_signal_job' => $d,
                                            ]),
                                            'edit_route' => route('admin.gps-remove-stop-signal-jobs.edit', [
                                                'gps_remove_stop_signal_job' => $d,
                                            ]),
                                            'view_permission' =>
                                                Actions::View . '_' . Resources::GPSRemoveStopSignalJob,
                                            'manage_permission' =>
                                                Actions::Manage . '_' . Resources::GPSRemoveStopSignalJob,
                                        ])
                                    @else
                                        @include('admin.components.dropdown-action', [
                                            'view_route' => route('admin.gps-remove-stop-signal-jobs.show', [
        ...
        @include('admin.gps-remove-stop-signal-jobs.modals.send-remove')
        @include('admin.gps-remove-stop-signal-jobs.modals.send-stop')
    </div>
</div>
@endsection
...
<script>
    ...
    function openModalSendRemove() {
        var enum_wait_remove = '{{ \App\Enums\GPSStopStatusEnum::WAIT_REMOVE_GPS }}';
        ...
        if (check_list.data.length > 0) {
            check_list.data.forEach(function(item, index) {
                ...
                if (is_check) {
                    if (item.remove_status == enum_wait_remove) {
                        sendCarToRemoveVue.addByDefault(item);
                    }
                }
            });
        }
        $('#modal-send-remove-siganl').modal('show');
    }

    function openModalSendStop() {
        var enum_wait_stop = '{{ \App\Enums\GPSStopStatusEnum::WAIT_STOP_SIGNAL }}';
        var check_list = @json($list);
        ...
        if (check_list.data.length > 0) {
            check_list.data.forEach(function(item, index) {
                ...
                var is_check = this_checkbox.prop('checked');
                if (is_check) {
                    if (item.stop_status == enum_wait_stop) {
                        sendCarToStopVue.addByDefault(item);
                    }
                }
            });
        }
        $('#modal-send-stop-siganl').modal('show');
    }
</script>
@endpush

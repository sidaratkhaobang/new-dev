@push('scripts')
    <script>
        function updateBorrowCarStatus(data) {
            var updateUri = "{{ route('admin.accident-order-sheet-approves.update-status') }}";
        ...
        $(".btn-not-approve-status").on("click", function() {
            var data = {
                ...
                redirect: "{{ route('admin.accident-order-sheet-approves.index') }}",
                approve_line_id: document.getElementById("approve_line").value,
                offer: $(this).attr('data-offer'),
                remark: document.getElementById("remark").value,
            };
            mySwal.fire({
                title: 'ยืนยันการส่งเสนอ GM',
                html: 'กรุณาให้เหตุผลการส่งเสนอ GM ในครั้งนี้ <span class="text-danger">*</span>',
                input: 'text',
                icon: 'warning',
                customClass: {
                    confirmButton: 'btn btn-danger m-1',

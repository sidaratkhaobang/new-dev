<div class="block {{ __('block.styles') }}">
    ...
    <div class="block-content">
        <div class="row push mb-4">
            ...
            <div class="col-sm-3">
                <x-forms.select-option id="cradle_area_id" :value="$d->cradle_area_id" :list="$province_list" :label="__('accident_orders.garage_area')" />
            </div>
            <div class="col-sm-3">
                {{-- <x-forms.select-option id="is_except_deductible" :value="$d->is_except_deductible" :list="[]" :label="__('accident_orders.garage_quotation')"
                    :optionals="['required' => true]" /> --}}

                @if (isset($view))
                    <x-forms.view-image :id="'garage_quotation_file'" :label="__('accident_orders.garage_quotation')" :list="$garage_quotation_file" />
                @else
                    <x-forms.upload-image :id="'garage_quotation_file'" :label="__('accident_orders.garage_quotation')" :optionals="['required' => true]" />
                @endif
            </div>
        </div>
        <div class="row push mb-4">
            ...
            @if ($accident_order->status == AccidentRepairStatusEnum::PROCESS_REPAIR)
                <div class="col-sm-3">
                    <x-forms.date-input id="actual_repair_date" :value="$d->actual_repair_date" :label="__('accident_orders.actual_repair_date')"
                        :optionals="['required' => true]" />
                </div>
            @endif
        </div>
